...
async function updateVersion() {
  try {
    ...
    try {
      ...
      readmeContent = fs.readFileSync('readme.txt', 'utf8');
    } catch (err) {
      if (err.code === 'ENOENT') {
        console.warn('Warning: readme.txt not found. Skipping Stable tag update.');
      } else {
        throw err;
      }
    }
    ...
    if (readmeContent) {
      ...
      if (stableTagMatch) {
        ...
        console.warn('Warning: No Stable tag found in readme.txt.');
      }
    }
